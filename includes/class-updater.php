...
class Updater
{
    ...
    public function get_github_response()
    {
        ...
        $this->access_token = get_option('gf_booking_github_token');
        ...
        $response = wp_remote_get(
            'https://api.github.com/repos/' . GFORM_BOOKING_GITHUB_REPO . '/releases/latest',
            $args
        );
    ...
    public function plugin_popup($result, $action, $args)
    {
        ...
        $changelog      = '';
        $changelog_file = GFORM_BOOKING_PLUGIN_DIR . 'CHANGELOG.md';
        ...
        if (empty($changelog)) {
            $changelog = $this->github_response->body ?: esc_html__('No changelog available.', 'gform-booking');
        }

        // Get README content for description.
        $description = $this->plugin['Description'];
        $readme_file = GFORM_BOOKING_PLUGIN_DIR . 'README.md';
    ...
    {
        return '<h3>' . esc_html__('Installation', 'gform-booking') . '</h3>
        <ol>
            <li>' . esc_html__('Download the latest release ZIP file', 'gform-booking') . '</li>
            <li>' . esc_html__('Go to WordPress Admin → Plugins → Add New → Upload Plugin', 'gform-booking') . '</li>
            <li>' . esc_html__('Upload the ZIP file and click Install Now', 'gform-booking') . '</li>
            <li>' . esc_html__('Activate the plugin', 'gform-booking') . '</li>
            <li>' . esc_html__('Configure services under GF Booking → Services', 'gform-booking') . '</li>
        </ol>';
    }
    ...
    {
        if ($this->active) {
            delete_transient('gf_booking_update_' . $this->basename);
        }
    }
}
